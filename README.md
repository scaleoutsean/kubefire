...
SolidFire CSI is an alternative CSI driver for SolidFire that, among other things, makes it easier to handle storage cluster failover and failback.
...
Unless you have no choice, I'd recommend to skip considering using Trident CSI with SolidFire in storage replication scenarios. You may want to look into the improvements related to the volume import feature of Trident (no rename, etc.) from recent years - maybe that will help you a bit, if it at all works with `solidfire-san`.
